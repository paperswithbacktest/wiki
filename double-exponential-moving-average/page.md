...
To calculate a Double Exponential Moving Average (DEMA), you first need to calculate an Exponential Moving Average (EMA). An EMA gives more weight to recent prices. You pick a time period, like 20 days, and then use a formula to figure out the EMA. The formula for the first EMA is: EMA1 = (Price today x K) + (EMA1 yesterday x (1 - K)), where K is a smoothing factor calculated as 2 / (Time period + 1). So, for a 20-day EMA, K would be 2 / (20 + 1) = 0.0952.
