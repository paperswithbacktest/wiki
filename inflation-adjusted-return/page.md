...
Bonds, on the other hand, often struggle to keep up with inflation. They pay a fixed [interest rate](/wiki/interest-rate-trading-strategies), so if inflation goes up, the value of those interest payments goes down. For instance, if a bond pays 3% interest but inflation is 4%, the real return is actually -1%. This means you're losing purchasing power. Some bonds, like Treasury Inflation-Protected Securities (TIPS), are designed to adjust with inflation, but regular bonds might not do as well in high inflation environments.
