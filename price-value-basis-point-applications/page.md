...
PVBP, or Price Value of a Basis Point, and duration are both used to measure how sensitive a bond's price is to changes in interest rates, but they do it in different ways. PVBP tells you how much the price of a bond will change if the [interest rate](/wiki/interest-rate-trading-strategies) goes up or down by one basis point, which is 0.01%. It's like a small snapshot of how the bond will react to a tiny change in rates. For example, if a bond's PVBP is $10, it means the bond's price will go up by $10 if the interest rate drops by one basis point, and it will go down by $10 if the rate goes up by one basis point.
