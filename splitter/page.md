...
In [machine learning](/wiki/machine-learning), the most common types of splitters used in decision trees are those based on information gain and Gini impurity. Information gain measures how much a split reduces the uncertainty in the dataset. It's calculated by comparing the entropy of the parent node to the weighted average entropy of the child nodes. If a split results in child nodes that are more certain about their outcomes, it has a high information gain and is considered a good split. On the other hand, Gini impurity measures how often a randomly chosen element from the set would be incorrectly labeled if it was randomly labeled according to the distribution of labels in the subset. A lower Gini impurity means a better split, as it indicates more homogeneous groups.
