...
The concept of pair trading can be traced back to its theoretical foundations in statistical arbitrage and mean reversion dynamics. It relies on the historical correlation between two assets — typically stocks, though it can also extend to other financial instruments such as futures, currencies, and exchange-traded funds (ETFs). The core principle is that, while the prices of these assets may drift apart in the short term, their long-term price relationship will remain stable and predictable.
