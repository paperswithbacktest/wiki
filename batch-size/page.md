...
Batch size, mini-batch size, and stochastic gradient descent are all related to how a machine learning model learns from data. Batch size is the number of training examples used in one update of the model. If you use all your data at once, that's called batch gradient descent. Mini-batch size is a smaller number of examples used in each update, usually between 1 and the total number of examples. This is called mini-batch gradient descent. Stochastic gradient descent (SGD) is when you use just one example at a time to update the model.
