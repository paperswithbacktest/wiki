...
When a company decides to update the value of an asset because its value has gone up, it starts the process of recording a revaluation reserve. First, the company gets a new value for the asset, usually by hiring someone who knows how to value assets, like an appraiser. The company then compares this new value to the old value that was on the [books](/wiki/algo-trading-books). If the new value is higher, the difference between the two values is the revaluation gain. This gain is what the company records in the revaluation reserve.
