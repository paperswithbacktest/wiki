...
Mini-batch is often preferred in training neural networks because it balances speed and accuracy well. When you use mini-batch, you're not using the whole dataset like in batch gradient descent, so it's faster. But you're also not using just one example like in stochastic gradient descent, which can make the training process noisy. Mini-batch uses a small group of examples, which makes the updates to the neural network more stable and helps it learn better. This method can use the power of modern hardware like GPUs, which can process many examples at once, making training even faster.
...
Mini-batch gradient descent is a way to train machine learning models, like neural networks, by using small groups of data called mini-batches. Instead of using the whole dataset at once, which can be slow, or just one example at a time, which can be noisy, mini-batch gradient descent uses a small group of examples. This helps the model learn faster and more smoothly. The size of the mini-batch is important and can be adjusted to find the best balance between speed and how well the model learns.
