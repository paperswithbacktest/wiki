...
To help avoid getting stuck in local minima, people use different tricks. One common trick is to use a technique called "momentum." Momentum helps the model keep moving even if it hits a small dip, kind of like a ball rolling down a hill. It keeps the model going in the right direction and can help it jump out of local minima to find a better spot. Another trick is to start the model from different places on the "landscape" and see which starting point leads to the best result. By trying different starting points, you increase the chance of finding the global minimum, the very best solution for your model.
