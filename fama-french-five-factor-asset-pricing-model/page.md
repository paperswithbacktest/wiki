...
The Five-Factor Model and the Three-Factor Model both help investors understand why some stocks do better than others. The Three-Factor Model, created by Eugene Fama and Kenneth French, looks at three things: the market, size, and value. The market factor means that when the whole stock market goes up, most stocks go up too. The size factor means that smaller companies might do better than bigger ones. The value factor means that stocks that seem cheaper might do better. This model was a big step forward in understanding stock prices.
