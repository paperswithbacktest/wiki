...
But currency swaps also have some downsides. They can be complicated to set up and might need a lot of paperwork. They also usually involve some costs, like fees or [interest rate](/wiki/interest-rate-trading-strategies) differences between the two currencies. If the exchange rate moves in a way the company didn't expect, it might end up losing out on potential gains. For example, if the other currency gets weaker, the company would have been better off not using the swap. So, while currency swaps can help manage risk, they're not perfect and need to be used carefully.
